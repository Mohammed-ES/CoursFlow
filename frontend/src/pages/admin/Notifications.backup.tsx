  ...
  const handleDelete = (notification: Notification) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await axios.delete(
            `${API_BASE_URL}/admin/notifications/${notification.id}`,
            { headers: getAuthHeaders() }
          );
          setAlertDialog({
            isOpen: true,
  ...
  const handleTogglePublish = async (notification: Notification) => {
    try {
      if (notification.published) {
        await axios.post(
          `${API_BASE_URL}/admin/notifications/${notification.id}/unpublish`,
          {},
          { headers: getAuthHeaders() }
        );
      } else {
        await axios.post(
          `${API_BASE_URL}/admin/notifications/${notification.id}/publish`,
          {},
          { headers: getAuthHeaders() }
        );
      }
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => setSelectedNotification(notification) || setShowViewModal(true)}
